...
export default class RoomClient {
  ...
  async enableWebcam() {
    ...
    try {
      ...
      if (this._forceH264) {
        codec = this._preciselyDevice.rtpCapabilities.codecs.find(
      ...
      if (this._useSimulcast) {
        ...
        const firstVideoCodec = this._preciselyDevice.rtpCapabilities.codecs.find(
          (c) => c.kind === "video"
        );
  ...
  // BACKGROUND TOGGLE FOR CUSTOM BACKGROUND
  async customBackground() {
    logger.debug("customBackground()");
    if (!this._webcamProducer) return;

    store.dispatch(stateActions.setCanChangeBackground(true));
  }
  async disableCustomBackground() {
    logger.debug("customBackground()");
    if (!this._webcamProducer) return;

    store.dispatch(stateActions.setCanChangeBackground(false));
  }
