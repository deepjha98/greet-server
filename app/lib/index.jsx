...
const store = createReduxStore(
  reducers,
  undefined,
  composeEnhancers(applyReduxMiddleware(...reduxMiddlewares))
);
    ...
    if (localStorage.getItem("auth-token")) {
      try {
        const { data } = await axios.get(
          `https://precisely.one/api/v1/verify_access_token_validity_for_event?room_id=${localStorage.getItem(
            "roomId"
          )}`,
          {
            headers: {
        ...
        if (error.response.status === 401) {
          swal({
            title: "Unauthorised",
            text:
              "Access token invalid, redirecting to https://app.precisely.one/",
            icon: "error",
            buttons: true,
            dangerMode: true,
          }).then(() => {
